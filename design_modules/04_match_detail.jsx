...
const U=[
  ...
  {id:8,name:"Burak Şen",un:"buraksen",av:"BŞ",att:90,follow:false,level:"mid"},
...
function S11({onNav}){
  ...
  const PlayerRow=({uid,team})=>{
    ...
    return <div onClick={()=>!u.guest&&onNav("S16",uid)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${T.cardBorder}22`,cursor:u.guest?"default":"pointer",opacity:isNoShow?.5:1}}>
      <Av i={u.av} s={32} c={isMvp?T.gold:team==="A"?T.accent:T.orange}/>
      <div style={{flex:1}}>
    ...
    {mvps.length>0&&<div style={{margin:"0 16px 16px",background:`${T.gold}08`,borderRadius:14,border:`1.5px solid ${T.gold}22`,padding:"14px 16px",textAlign:"center"}}>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:16}}>
        {mvps.map(u=><div key={u.id} onClick={()=>onNav("S16",u.id)} style={{cursor:"pointer",textAlign:"center"}}>
          <Av i={u.av} s={48} c={T.gold} st={{margin:"0 auto",border:`2px solid ${T.gold}`}}/>
...
function S12({onNav}){
  ...
  const host=uf(m.host);
  const isHost=m.host===1;
  const isPlayer=m.players.includes(1);
    ...
    <div style={{padding:"0 16px 16px"}}>
      ...
      <div style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontSize:13,fontWeight:600,color:T.text}}>{m.joined}/{m.max} oyuncu</span>
          <span style={{fontSize:12,color:remaining<=2?T.orange:T.textDim}}>{remaining} yer kaldı</span>
        </div>
        <CapacityBar joined={m.joined} max={m.max}/>
      </div>

      {/* Host takeover banner (if active) */}
      {m.hostTakeover&&<div style={{background:`${T.gold}10`,borderRadius:12,border:`1.5px solid ${T.gold}33`,padding:"12px 16px",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
          {I.crown(T.gold)}
          <span style={{fontSize:13,fontWeight:700,color:T.gold}}>{uf(m.hostTakeover.requester)?.name} host olmak istiyor</span>
        </div>
        <div style={{fontSize:12,color:T.textDim,marginBottom:10}}>Kalan süre: {m.hostTakeover.deadline} · {m.hostTakeover.votesYes+m.hostTakeover.votesNo}/{m.hostTakeover.total} oy</div>
        <div style={{display:"flex",gap:8}}>
          <Btn small primary>Evet</Btn>
          <Btn small danger>Hayır</Btn>
        </div>
      </div>}
    </div>
      ...
      {pending.length>0?pending.map(app=>{
        ...
        return <div key={app.uid} style={{background:T.card,borderRadius:12,border:`1px solid ${T.cardBorder}`,padding:"14px 16px",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            <Av i={u.av} s={40} onClick={()=>onNav("S16",u.id)}/>
            <div style={{flex:1}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:20}}>
        {m.players.filter(uid=>uid!==1).map(uid=>{
          ...
          return <div key={uid} onClick={()=>setMvpVote(sel?null:uid)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:12,background:sel?`${T.gold}10`:T.card,border:`1.5px solid ${sel?T.gold:T.cardBorder}`,cursor:"pointer",transition:"all .2s"}}>
            <Av i={u.av} s={32} c={sel?T.gold:T.accent}/>
