...
function Btn({ children, primary, small, full, onClick, st }) {
  ...
  return <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{ padding:small?"6px 16px":"12px 20px", borderRadius:10, border:primary?"none":`1.5px solid ${T.cardBorder}`, background:primary?T.accent:"transparent", color:primary?T.onAccent:T.text, fontSize:small?12:14, fontWeight:600, cursor:"pointer", width:full?"100%":"auto", transition:"all .2s", transform:h?"translateY(-1px)":"none", display:"flex", alignItems:"center", justifyContent:"center", gap:8, ...st }}>{children}</button>;
}
...
function MediaSlider({ photoCount, imgs, scoreContent, onMatchNav }) {
  ...
  return <div style={{ position:"relative", overflow:"hidden", height:430 }}>
    ...
    {/* Sayaç */}
    <div style={{ position:"absolute", top:10, right:10, background:T.overlayDark, borderRadius:20, padding:"3px 9px", fontSize:11, color:T.white, fontWeight:600 }}>
...
export default function SporWaveFeed() {
  ...
  return <div style={{ maxWidth:430, margin:"0 auto", minHeight:"100vh", background:T.bg, color:T.text, fontFamily:FONT_B, position:"relative", boxShadow:T.shadowPage }}>
    {/* Dev ribbon */}
    <div style={{ position:"sticky", top:0, zIndex:200, background:T.bgAlt, borderBottom:`1px solid ${T.cardBorder}`, padding:"8px 8px", display:"flex", gap:4 }}>
      {[{p:"S05",l:"Feed"},{p:"S07",l:"Arama"},{p:"S42",l:"Yorumlar",id:101},{p:"S43",l:"Beğeniler",id:101}].map(n=><span key={n.p} onClick={()=>nav(n.p,n.id)} style={{ padding:"4px 12px", borderRadius:8, fontSize:11, fontWeight:600, background:cur===n.p?T.accent:`${T.textDim}22`, color:cur===n.p?T.onAccent:T.textDim, cursor:"pointer" }}>{n.l}</span>)}
    </div>
